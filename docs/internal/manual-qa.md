...
| QA-85 | Hoch | Den echten macOS-Accessibility-Smoke gegen ein isoliertes Test-Bundle mit den versionierten PDF-Fixtures ausführen. | Kapitelansprung, lokale Lesezeichen, wiederhergestellte Leseposition sowie sämtliche vorhandenen Formular-, Zuschnitt- und Schutzfälle bestehen; Original-Fixtures und Nutzerdokumente bleiben unverändert. | Terminalausgabe + Fixture-Dateihashes |
...
| ID | Prio | Aktion | Pass-Kriterium | Evidenz |
| --- | --- | --- | --- | --- |
| QA-87 | Kritisch | Crop-Fixture öffnen; Lesemodus, Menü, Tastatur und Accessibility prüfen. | Kein schreibender Crop-Einstieg; erst `Bearbeiten` zeigt `Seite zuschneiden …` im Bereich `Seiten`. | Echter AX-Smoke + Store-Test |
| QA-88 | Hoch | Auf jeder der vier Seiten das Sheet öffnen; neuen Rahmen ziehen und Ecken verändern. | Vollständige MediaBox-Vorschau und Rahmen stimmen bei 0/90/180/270 Grad trotz versetztem Ursprung mit dem PDF überein. Außenfläche ist abgedunkelt, Randwerte folgen der sichtbaren Orientierung. | Sichtprüfung + PDFView-Konvertierungstest |
| QA-89 | Kritisch | Rahmen über Seitenränder oder unter die Mindestgröße ziehen; ungültige Geometrie an Store übergeben. | Rahmen bleibt gültig; Mindestmaß 36 pt oder volle kleinere Originaldimension. Nicht endliche, negative und außerhalb liegende Store-Eingaben ändern nichts. | Geometrie-/Store-Tests |
| QA-90 | Kritisch | Rahmen verändern und `Abbrechen` wählen, sowohl bei sauberem als auch bereits ungespeichertem Dokument. | CropBox, Inhalte und bisheriger Dirty-State bleiben erhalten. | AX-Smoke + Store-Test |
| QA-91 | Kritisch | Auf jeder Drehung asymmetrische Ränder anwenden; andere Seiten, Miniatur, Ansicht und Größe prüfen. | Nur aktuelle CropBox geändert, Dirty-State sichtbar, alle Anzeigen aktualisiert, andere Seiten unverändert. | AX-Smoke + Store-Test + Sichtprüfung |
| QA-92 | Kritisch | Nach Zuschnitt schließen oder Dokument wechseln und abbrechen; dann speichern und erneut öffnen. | Dokumentenschutz bleibt aktiv; gespeicherter relativer Ausschnitt und Rotation bleiben erhalten; erneutes Öffnen beginnt im Lesemodus. | AX-Smoke + Save/Reopen |
| QA-93 | Kritisch | Erneut zuschneiden, danach `Auf Seitengröße zurücksetzen`, speichern und erneut öffnen. | CropBox entspricht aktueller MediaBox; vollständige Inhalte bleiben verfügbar. PDFKit darf den absoluten Ursprung beim Export normalisieren, nicht die Größe oder den relativen Ausschnitt verändern. | AX-Smoke + Fixture-Inhaltsvergleich |
